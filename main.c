...
// prototypes
void get_api_token(char answer);
...
size_t save_access_token(char *ptr, size_t size, size_t nmemb, void *not_used)
{
    ...
    fprintf(access_token_file, "%s", ptr);
    fclose(access_token_file);

    parsed_json = json_tokener_parse(ptr);
    access_token = json_object_object_get(parsed_json, "result");

    char *access_token_string = malloc(sizeof(char) * BUFFER_SIZE);
...
void get_api_token(char answer)
{
    ...
    if (answer == 'y')
    {
        ...
        if (curl)
        {
            ...
            curl_easy_setopt(curl, CURLOPT_WRITEFUNCTION, save_access_token); // this is saved into the save_access_token function as a *ptr
